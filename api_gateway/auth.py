...
# Removed werkzeug.security imports, will use passlib
from db import SessionLocal, engine
from models import Base, Route, RequestLog, User # Import User model
...
from config import (
    ...
    MAX_CONTENT_LENGTH # Removed REQUIRE_USER_AUTH, VALID_USERS
)
...
# Only create tables if explicitly requested (handled by init_postgres.py now)
...
def login():
    ...
    try:
        ...
        # Generate tokens with roles from DB
...
#@token_required # This endpoint should NOT be token_required for initial token issuance
def issue_token():
    ...
    try:
        ...
        # In this context, "user" parameter is treated as username for legacy compatibility
        # Authenticate user against database (basic check for existence and active status)
        user = db.query(User).filter_by(username=username).first()
        if not user or not user.is_active:
            return jsonify({"error": "User not found or inactive"}), 401

        tokens = generate_tokens(user.username, roles=user.roles) # Fetch roles from DB
